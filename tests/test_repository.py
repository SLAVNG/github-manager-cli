...
#     # Mock Github instance and its methods
...
@patch("ghrm.repository.get_github_and_org")
def test_get_repo_success(mock_get_github_and_org):
    """Test fetching an existing repository."""
    mock_org = Mock()
    mock_repo = Mock()
    mock_repo.name = "test_repo"
    mock_org.get_repo.return_value = mock_repo
    mock_get_github_and_org.return_value = (Mock(), mock_org)
...
@patch("ghrm.repository.get_github_and_org")
def test_get_repo_not_found(mock_get_github_and_org):
    """Test fetching a non-existent repository."""
    mock_org = Mock()
    mock_org.get_repo.side_effect = GithubException(404, "Repository not found", None)
    mock_get_github_and_org.return_value = (Mock(), mock_org)
...
@patch("ghrm.repository.get_github_and_org")
def test_create_repository(mock_get_github_and_org, mock_get_repo):
...
@patch("ghrm.repository.get_repo")
@patch("ghrm.repository.get_github_and_org")
def test_create_repository_already_exists(mock_get_github_and_org, mock_get_repo):
    """Test creating a repository that already exists."""
    mock_repo = Mock()
    mock_get_repo.return_value = mock_repo
    mock_get_github_and_org.return_value = (Mock(), Mock())
